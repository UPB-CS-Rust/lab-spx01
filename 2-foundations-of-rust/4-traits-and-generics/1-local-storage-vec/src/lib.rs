...
fn move_to_vec<T, const N: usize>(stack: &mut [T; N]) -> Vec<T>
where
    T: Default,
{
    let mut vec: Vec<T> = Vec::with_capacity(N);
    for i in 0..N {
        vec.push(mem::take(&mut stack[i]));
    }
    vec
}
    ...
    T: Default,
{
            ...
            LocalStorageVec::Stack { buf, len } => {
                if *len < N {
                    ...
                    *len += 1;
                    ...
                    let mut vec = move_to_vec(buf);
                    vec.push(value);
                    *self = LocalStorageVec::Heap(vec);
                }
            }
            LocalStorageVec::Heap(vec) => vec.push(value),
        }
    }
    ...
    fn insert(&mut self, index: usize, value: T) {
        match self {
            ...
            LocalStorageVec::Heap(vec) => vec.insert(index, value),
        }
    }
    ...
    fn remove(&mut self, index: usize) -> T {
        match self {
            LocalStorageVec::Stack { buf, len } => {
                *len -= 1;
                let value = mem::take(&mut buf[index]);
                for i in index..*len {
                    buf[i] = mem::take(&mut buf[i + 1]);
                }
                value
    ...
    T: Default,
{
    ...
    fn into_iter(self) -> Self::IntoIter {
        match self {
            LocalStorageVec::Stack { mut buf, len } => {
                let mut vec = Vec::with_capacity(len);
                for i in 0..len {
                    vec.push(mem::take(&mut buf[i]));
                }
                vec.into_iter()
            }
            LocalStorageVec::Heap(vec) => vec.into_iter(),
        }
    }
}
...
impl<T, const N: usize> Index<RangeFrom<usize>> for LocalStorageVec<T, N> {
    ...
    fn index(&self, range: RangeFrom<usize>) -> &Self::Output {
        match self {
            LocalStorageVec::Stack { buf, len } => &buf[range.start..*len],
...
impl<T, const N: usize> Index<RangeTo<usize>> for LocalStorageVec<T, N> {
    ...
    fn index(&self, range: RangeTo<usize>) -> &Self::Output {
        match self {
            LocalStorageVec::Stack { buf, .. } => &buf[range],
            LocalStorageVec::Heap(vec) => &vec[range],
        }
    }
}
